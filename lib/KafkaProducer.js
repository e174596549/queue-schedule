...
class KafkaProducer extends EventEmitter {
    ...
    constructor({
        ...
        if (!producer) {
            if (!zookeeperHost && !kafkaHost) {
                throw new Error('You must giva a producer or zookeeperHost or kafkaHost.');
            }
            this._createProducer(zookeeperHost,kafkaHost);
        } else {
            this._clientOk = true;
    ...
    _createProducer(zookeeperHost,kafkaHost) {
        const client = kafkaHost ? 
        new kafka.KafkaClient({kafkaHost: kafkaHost})
        : new kafka.Client(zookeeperHost);
        const _this = this;
    ...
    _sendAtOnce(taskData,callback) {
        const _this = this;
        this.producer.send(
            this._createSendData(JSON.stringify(taskData)), function(err,data) {
                callback(err,data);
                if (err) {
