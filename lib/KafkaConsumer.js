 ...
 * 
 * @param {String=} kafkaHost connect directly to kafka broker (instantiates a KafkaClient), it will use KafkaConsumerOption.kafkaHost in default.
 * @param {Object=} zk put client zk settings if you need them
 * @param {Object=} batch  put client batch settings if you need them (see Client)
...
class KafkaConsumer extends EventEmitter {
    ...
    constructor({
        ...
        this.consumer = null;
        if ((!kafkaHost) || !topics ||  !consumerOption) {
            throw new Error('One of parameters of kafkaHost,  topics and consumerOption must be given');
        }
