...
class KafkaConsumer extends EventEmitter {
    ...
    constructor({
        ...
        if (!consumer) {
            if (!zookeeperHost || !topics ||  !consumerOption) {
                throw new Error('One of parameters of consumer or zookeeperHost, topics and consumerOption must be given');
            }
    ...
    _init(zookeeperHost,topics,consumerOption) {
        ...
        const consumer = this.consumer = new kafka.Consumer(
            client, topics, consumerOption
        );
